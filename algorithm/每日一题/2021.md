...
```
## 696.重复叠加字符串
题目描述：[链接](https://leetcode-cn.com/problems/repeated-string-match/)
```cpp
class Solution {
public:
    int repeatedStringMatch(string a, string b) {
        ...
        for(int i=0;i<len_b;++i){
            if(set_ch.find(b[i]) == set_ch.end())
                return -1;
        }
        int min_multi = len_b / len_a;
        string str = "";
...
```
## 1705.吃苹果的最大数目
题目描述：[链接](https://leetcode-cn.com/problems/maximum-number-of-eaten-apples/)
```cpp
// My stupid method
class Solution {
public:
    int eatenApples(vector<int>& apples, vector<int>& days) {
        int len = apples.size();
        int total_eat = 0;
        int min_num = INT_MAX; // 最先腐烂的剩余天数
        int min_position = INT_MAX;// 最先腐烂的剩余天数对应的下标
        int day = 1; //已经进行的天数
        int have = 0; // 最早还未腐烂的苹果的下标
        int flag = 0;
        do {
            min_num = INT_MAX;
            flag = 0;
            for(int j=have;j< (day>len ? len : day);++j){
                if(days[j] > 0 && apples[j] > 0){
                    if(flag == 0) have = j;
                    flag = 1;
                    if(min_num > days[j]){
                        min_num = days[j];
            ...
            if(min_num != INT_MAX){
                apples[min_position] -= 1;
                total_eat += 1;
            }
            ++day;
        }while((min_num != INT_MAX) || (day < len+1));
        return total_eat;
    }
};
```
## 846.一手顺子
题目描述：[链接](https://leetcode-cn.com/problems/hand-of-straights/)
```cpp
class Solution {
public:
...
```
## 507.完美数
题目描述：[链接](https://leetcode-cn.com/problems/perfect-number/)
```cpp
class Solution {
public:
